    ...
    public static class Builder {
        ...
        public Builder() {
            if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
                dir = TKBaseApplication.myApplication.getApplicationContext().getExternalCacheDir() + File.separator + "log" + File.separator;

            } else {
                dir = TKBaseApplication.myApplication.getApplicationContext().getCacheDir() + File.separator + "log" + File.separator;
            }
        }
...
//        if (AppUtils.isAppDebug(MyApp.app)) {
//            PublicPracticalMethod.getInstance().writePrintLog(msg);//暂时屏蔽掉写入日志文件到本机中
//        }
        if (sLogSwitch) {
