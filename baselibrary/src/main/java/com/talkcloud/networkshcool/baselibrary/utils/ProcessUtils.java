...
public class ProcessUtils {
    ...
    public static String getForegroundProcessName() {
        ActivityManager manager = (ActivityManager) TKBaseApplication.myApplication.getApplicationContext().getSystemService(Context.ACTIVITY_SERVICE);
        ...
        if (android.os.Build.VERSION.SDK_INT > android.os.Build.VERSION_CODES.LOLLIPOP) {
            PackageManager packageManager = TKBaseApplication.myApplication.getApplicationContext().getPackageManager();
            Intent intent = new Intent(Settings.ACTION_USAGE_ACCESS_SETTINGS);
            List<ResolveInfo> list = packageManager.queryIntentActivities(intent, PackageManager.MATCH_DEFAULT_ONLY);
            System.out.println(list);
            if (list.size() > 0) {// 有"有权查看使用权限的应用"选项
                try {
                    ApplicationInfo info = packageManager.getApplicationInfo(TKBaseApplication.myApplication.getApplicationContext().getPackageName(), 0);
                    AppOpsManager aom = (AppOpsManager) TKBaseApplication.myApplication.getApplicationContext().getSystemService(Context.APP_OPS_SERVICE);
                    ...
                    UsageStatsManager usageStatsManager = (UsageStatsManager) TKBaseApplication.myApplication.getApplicationContext().getSystemService(Context.USAGE_STATS_SERVICE);
    ...
    public static Set<String> getAllBackgroundProcesses() {
        ActivityManager am = (ActivityManager) TKBaseApplication.myApplication.getApplicationContext().getSystemService(Context.ACTIVITY_SERVICE);
